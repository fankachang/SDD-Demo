<!--
同步影響報告：
- 版本：1.0.1 → 1.1.0（次版本：新增不可協商原則『避免過度開發與過度設計』）
- 修改原則：無（未更動條款內容）
- 新增章節：無
- 移除章節：無
- 已檢查模板：✅ .specify/templates/plan-template.md
...
現有測試**必須**在變更後持續通過。新功能**應該**在被要求時包含測試。**絕對禁止**修改或移除現有測試檔案，除非被明確指示。
...
**必須**遵循程式碼庫中現有的模式和命名慣例。**必須**使用
專案既有的依賴套件。**絕對禁止**在未經明確批准的情況下升級主要版本或引入新框架。
...
團隊**必須**避免實作超出使用者明確要求的功能或引入不必要的複雜性。實作**必須**
以最簡單且穩健的解法滿足驗收標準與成功指標。未經明確需要或未經詳實需求文件
證明之投機性設計、過度工程或超出範圍的擴充**禁止**實施，除非獲得使用者明確批准。

**理由說明**：過度開發與過度設計會增加維護成本、引入額外風險、延緩交付，並違反
最小化變更原則。在棕地專案中，優先事項為在最少干擾下交付可衡量的使用者價值。
...
**版本**：1.1.0 | **批准日期**：2025-12-07 | **最後修訂**：2025-12-07