...
def list_programs(db: Session = Depends(db.get_db)):
    return db.query(models.Program).all()


@router.post("/programs", status_code=201, response_model=schemas.Program)
def create_program(payload: schemas.Program, db: Session = Depends(db.get_db)):
...
def update_program(id: int, payload: schemas.Program, db: Session = Depends(db.get_db)):
...
def delete_program(id: int, db: Session = Depends(db.get_db)):
    p = db.query(models.Program).filter(models.Program.id == id).first()
    if not p:
        raise HTTPException(status_code=404, detail="Program not found")
    db.delete(p)
